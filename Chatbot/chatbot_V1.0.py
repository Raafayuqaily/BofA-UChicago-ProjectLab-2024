    ...
    if user_question:
        if is_finance_related(user_question):
            ...
            if isinstance(results, pd.DataFrame) and not results.empty:
                ...
                analyze = st.radio("Would you like an analysis of this data?", ("No", "Yes"), key=len(st.session_state["analyses"]))
                if analyze == "Yes":
                    ...
                    st.session_state["analyses"].append({"question": user_question, "analysis": analysis})
                
                    # Option to save analysis as PDF
    ...
    if st.session_state["conversation"]:
        ...
        for idx, msg in enumerate(st.session_state["conversation"]):
            if msg["role"] == "user":
                ...
                # Provide option to download previous analyses
                if idx // 2 < len(st.session_state["analyses"]):
                    analysis_data = st.session_state["analyses"][idx // 2]
                    pdf_buffer = save_analysis_as_pdf(analysis_data["question"], analysis_data["analysis"])
                    st.download_button(
                        label=f"Download Analysis {idx // 2 + 1} as PDF",
                        data=pdf_buffer,
                        file_name=f"equity_analysis_{idx // 2 + 1}.pdf",
                        mime="application/pdf",
                        key=f"download_{idx}"
                    )
